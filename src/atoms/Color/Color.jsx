...
export default class Color extends React.Component {
    render(){
        ...
        const GlowRect = styled.div`
            ...
            ${this?.state?.isActive ? `
                -webkit-box-shadow: 0px 0px 10px 5px ${this.colorObj[this.color]} !important;
                -moz-box-shadow: 0px 0px 10px 5px ${this.colorObj[this.color]}!important;
                box-shadow: 0px 0px 10px 5px ${this.colorObj[this.color]}!important
            ` : ''}
        `
        ...
        return(
            <GlowRect onClick={onClick} className={style.border}>
                <InnerRect  className={`${style.innerRect} ${this.color}`}>
                </InnerRect>
            </GlowRect>
        )
        }
